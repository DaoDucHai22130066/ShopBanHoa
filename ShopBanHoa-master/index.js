...
window.onload = function () {
  ...
  if (!currentUser) {
    // Nếu chưa đăng nhập, giữ nguyên ở index.html
    console.log("Trang mặc định: Chưa đăng nhập");
  } else {
    // Nếu đã đăng nhập, giữ nguyên giao diện nhưng hiển thị thêm thông tin cá nhân
    document.getElementById("account-link").textContent =
      "Xin chào, " + currentUser.name;
    document.getElementById("logout-link").style.display = "inline-block";
  ...
  function login() {
    ...
    if (user) {
      // Nếu thông tin hợp lệ, lưu người dùng vào localStorage
      alert("Đăng nhập thành công!");
      ...
      document.getElementById("logout-link").style.display = "inline-block";
      document.getElementById("change-password-button").style.display =
        "inline-block"; // Hiển thị nút Đổi mật khẩu
      toggleModal(); // Đóng modal đăng nhập
    } else {
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      if (currentUser) {
        // Nếu đã đăng nhập, hiển thị modal chỉnh sửa tài khoản
        showEditAccount();
      } else {
